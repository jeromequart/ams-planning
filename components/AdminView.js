...
export default function AdminView({ salaries, setSalaries, evenements, inscriptions, setInscriptions, missionTypes }) {
  ...
  // Missions du salarié = événements où il est inscrit (validé)
  const mesInscriptions = inscriptions.filter(i => i.salarieId === selected && i.statut === 'valide');
  const mesEvenements = evenements
    .filter(e => mesInscriptions.find(i => i.evenementId === e.id))
  ...
  function openAdd() {
    setForm({ ...FORM_VIDE, colorIdx: salaries.length % AVATAR_COLORS.length });
    setEditId(null); setShowModal(true);
  }
  function openEdit(s) {
    setForm({ prenom:s.prenom, nom:s.nom, role:s.role, colorIdx:s.colorIdx });
    setEditId(s.id); setShowModal(true);
  }
  function save() {
    if (!form.prenom.trim() || !form.nom.trim()) return;
    if (editId) {
      setSalaries(prev => prev.map(s => s.id===editId ? {...s,...form} : s));
    } else {
      const n = { id:uuid(), ...form };
      setSalaries(prev => [...prev, n]);
      setSelected(n.id);
    }
    setShowModal(false);
  }
  function deleteSalarie(id) {
    if (!confirm('Supprimer ce salarié ? Ses inscriptions seront également supprimées.')) return;
    setSalaries(prev => prev.filter(s => s.id!==id));
    setInscriptions(prev => prev.filter(i => i.salarieId!==id));
    if (selected===id) setSelected(null);
  }
  function retirerDeMission(inscId) {
    ...
    <div style={s.container}>
      ...
      <div style={s.sidebar}>
        ...
        {salaries.map(sal => {
          ...
          return (
            <div key={sal.id} onClick={() => setSelected(sal.id)}
          ...
          <>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, paddingBottom:16, borderBottom:'1px solid var(--border)' }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                ...
                <button style={{ ...s.btnSec, color:'var(--red)' }} onClick={() => deleteSalarie(salarie.id)}>🗑 Supprimer</button>
              </div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:20 }}>
              {[
                { label:'Missions planifiées', value:mesEvenements.length },
            ...
            <div style={{ background:'#fff', borderRadius:12, border:'1px solid var(--border)', overflow:'hidden' }}>
              ...
              {mesEvenements.length === 0 ? (
                <div style={{ textAlign:'center', padding:'32px', color:'var(--text-3)', fontSize:13 }}>
                  Aucune mission assignée.<br/>
                  <span style={{ fontSize:12 }}>Créez des événements dans l'onglet Planning et inscrivez ce salarié.</span>
                </div>
              ) : (
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr>
                      {['Événement','Date','Horaires','Type','Lieu','Durée','Statut',''].map(h => (
                      ...
                      return (
                        <tr key={ev.id} style={{ background: estPassee ? '#fafaf8' : '#fff', opacity: estPassee ? 0.7 : 1 }}>
                          <td style={{ padding:'10px', borderBottom:'1px solid var(--border)', fontWeight:500 }}>
                            {ev.nom || '—'}
                            {ev.ref && <span style={{ display:'block', fontSize:10, color:'var(--text-3)', fontFamily:'var(--font-mono)' }}>{ev.ref}</span>}
                          </td>
                          ...
                          <td style={{ padding:'10px', borderBottom:'1px solid var(--border)' }}>
                            <span style={{ background:mt.bg, color:mt.color, fontSize:11, padding:'2px 8px', borderRadius:20, fontWeight:500, whiteSpace:'nowrap' }}>
                              {mt.icon} {mt.label}
                            </span>
                          </td>
                          <td style={{ padding:'10px', borderBottom:'1px solid var(--border)', color:'var(--text-2)', fontSize:12 }}>{ev.lieu || '—'}</td>
                          <td style={{ padding:'10px', borderBottom:'1px solid var(--border)', fontFamily:'var(--font-mono)', fontSize:12, whiteSpace:'nowrap' }}>{fmtH(dureeH(ev.debut,ev.fin))}</td>
                          <td style={{ padding:'10px', borderBottom:'1px solid var(--border)' }}>
                            {estPassee
        ...
        <Modal title={editId ? 'Modifier le salarié' : 'Nouveau salarié'} onClose={() => setShowModal(false)}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
            <div>
              ...
              <input style={s.input} value={form.prenom} onChange={e=>setForm(p=>({...p,prenom:e.target.value}))} placeholder="Prénom" autoFocus />
            </div>
            <div>
              <label style={s.label}>Nom *</label>
              <input style={s.input} value={form.nom} onChange={e=>setForm(p=>({...p,nom:e.target.value}))} placeholder="NOM" />
            </div>
          </div>
          <div style={{ marginBottom:14 }}>
            <label style={s.label}>Rôle / Poste</label>
            <input style={s.input} value={form.role} onChange={e=>setForm(p=>({...p,role:e.target.value}))} placeholder="Ex : Secouriste, Chef d'équipe…" />
          </div>
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:6 }}>
              {AVATAR_COLORS.map((c,i) => (
                <div key={i} onClick={() => setForm(p=>({...p,colorIdx:i}))}
                  style={{ width:32, height:32, borderRadius:'50%', background:c.bg, color:c.txt, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:600, cursor:'pointer', border:form.colorIdx===i?`2px solid ${c.txt}`:'2px solid transparent' }}>
          ...
          <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:20 }}>
            ...
            <button style={s.btnPri} onClick={save}>{editId ? 'Enregistrer' : 'Créer le salarié'}</button>
          </div>
        </Modal>
      )}
    </div>
